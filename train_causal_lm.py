...
# 这段Python代码 checkpoint_model_optimizer_scheduler 的作用是保存模型、优化器和学习率调度器的状态，以便在训练中断后可以从中断的地方继续恢复训练。它的主要功能就是进行“检查点”保存操作。
...
def checkpoint_model(accelerator, model, tokenizer, output_ckpt_dir, last_global_step):    
    ...
    if accelerator.is_main_process:
        ...
        # Manual saving of the full PyTorch state_dict
        # This assumes the model has been correctly aggregated by get_state_dict
        full_state_dict = accelerator.get_state_dict(unwrapped_model)
        pytorch_model_path = os.path.join(ckpt_path, "pytorch_model.bin")

        # Ensure all tensors are moved to CPU for saving
        full_state_dict = {k: v.cpu() for k, v in full_state_dict.items()}

        # Save the full PyTorch model
        torch.save(full_state_dict, pytorch_model_path)
...
def train(opt):
    ...
    model = AutoModelForCausalLM.from_pretrained(opt.pretrained_model_name_or_path, torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2")
    ...
    if opt.mode == "pt":
        ...
        dataset = SFTPLSQLGenerationDataset(opt.text2sql_data_dir, tokenizer, opt.block_size, "train", opt.table_num, opt.column_num, None, None, None, None, None)
    else:
