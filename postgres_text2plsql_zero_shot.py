...
from utils.postgres_db_utils import check_plsql_executability, compare_plsql, is_exact_match
...
if __name__ == "__main__":
    ...
    for raw_data, batch_data in tqdm(zip(raw_dataset, dataloader)):
        ...
        for generated_plsql in generated_plsqls:
            execution_error = check_plsql_executability(generated_plsql, raw_data["call"], raw_data["database"].lower())
    ...
    for idx, (raw_data, predicted_plsql) in enumerate(zip(raw_dataset, predicted_plsqls)):
        ...
        if compare_plsql(raw_data["database"], raw_data["plsql"], predicted_plsql, raw_data["call"], True):
